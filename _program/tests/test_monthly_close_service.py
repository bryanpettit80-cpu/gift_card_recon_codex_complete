...
@pytest.mark.parametrize(
    ...
    [
        (
            0x00000400 | 0x00080000 | 0x00100000,
            "both pinned and unpinned",
        ),
        (
            0x00000400 | 0x00001000,
            "offline Dropbox placeholder",
        ),
    ],
)
def test_invalid_dropbox_placeholder_blocks_publication_before_replacement(
    tmp_path: Path,
    monkeypatch,
    attributes: int,
    message: str,
    ...
    canonical_xlsx = tmp_path / "output" / "close.xlsx"
    canonical_pdf = tmp_path / "output" / "close.pdf"
    canonical_xlsx.parent.mkdir()
    staged_xlsx.write_bytes(b"new workbook")
    staged_pdf.write_bytes(b"new PDF")
    canonical_xlsx.write_bytes(b"old workbook")
    canonical_pdf.write_bytes(b"old PDF")

    real_attributes = service._windows_file_attributes

    def placeholder_attributes(path: Path) -> int | None:
        if Path(path) == canonical_xlsx:
            return attributes
        return real_attributes(path)

    monkeypatch.setattr(service, "_windows_file_attributes", placeholder_attributes)

    with pytest.raises(PermissionError, match=message):
        service._publish_pair_transactional(
            temp_xlsx=staged_xlsx,
            temp_pdf=staged_pdf,
    ...
    assert canonical_pdf.read_bytes() == b"old PDF"
    assert not list(canonical_xlsx.parent.glob(".*.publish"))
    assert not list(canonical_xlsx.parent.glob(".*.backup"))


def test_monthly_publication_retries_transient_dropbox_lock_and_verifies_hashes(
    tmp_path: Path,
    monkeypatch,
) -> None:
    from gift_card_recon import monthly_close_service as service
    ...
    def lock_workbook_once(source, destination):
        nonlocal attempts
        if Path(destination) == canonical_xlsx and Path(source).suffix == ".publish":
            attempts += 1
            if attempts == 1:
                exc = PermissionError(
                    errno.EACCES,
    ...
    monkeypatch.setattr(service.os, "replace", lock_workbook_once)
    ...
    assert canonical_xlsx.read_bytes() == staged_xlsx.read_bytes()
    assert canonical_pdf.read_bytes() == staged_pdf.read_bytes()
    assert not list(canonical_xlsx.parent.glob(".*.publish"))
    assert not list(canonical_xlsx.parent.glob(".*.backup"))


def test_exhausted_dropbox_lock_restores_existing_monthly_pair(
    tmp_path: Path,
    monkeypatch,
) -> None:
    from gift_card_recon import monthly_close_service as service
    ...
    def keep_pdf_locked(source, destination):
        nonlocal attempts
        if Path(destination) == canonical_pdf and Path(source).suffix == ".publish":
            attempts += 1
            exc = PermissionError(
                errno.EACCES,
                "simulated persistent Dropbox lock",
                str(destination),
            )
            exc.winerror = 32
            raise exc
    ...
    monkeypatch.setattr(service.os, "replace", keep_pdf_locked)
    monkeypatch.setattr(service.time, "sleep", retry_delays.append)

    with pytest.raises(PermissionError, match="persistent Dropbox lock"):
        service._publish_pair_transactional(
            temp_xlsx=staged_xlsx,
            temp_pdf=staged_pdf,
            canonical_xlsx=canonical_xlsx,
    ...
    assert attempts == len(service._TRANSIENT_FILE_RETRY_DELAYS) + 1
    assert retry_delays == list(service._TRANSIENT_FILE_RETRY_DELAYS)
    assert canonical_xlsx.read_bytes() == b"old workbook"
    assert canonical_pdf.read_bytes() == b"old PDF"
    assert not list(canonical_xlsx.parent.glob(".*.publish"))
    assert not list(canonical_xlsx.parent.glob(".*.backup"))
...
def test_hash_mismatch_in_publish_copy_preserves_existing_monthly_pair(
    tmp_path: Path,
    monkeypatch,
) -> None:
    from gift_card_recon import monthly_close_service as service
    ...
    def corrupt_workbook_copy(source, destination):
        result = real_copy2(source, destination)
        if Path(source) == staged_xlsx:
            Path(destination).write_bytes(b"corrupted copy")
        return result

    monkeypatch.setattr(service.shutil, "copy2", corrupt_workbook_copy)

    with pytest.raises(OSError, match="Hash verification failed while staging"):
        service._publish_pair_transactional(
            temp_xlsx=staged_xlsx,
            temp_pdf=staged_pdf,
            canonical_xlsx=canonical_xlsx,
